...
fn read_data<R: Read>(
    ...
    let measurements = Measurements {
        ...
        PWM_0-10V_A: fields
            .next()
            .ok_or_else(|| eyre!("Field `PWM_0-10V_A` not set."))?
            .raw_value_f64()
            .ok_or_else(|| eyre!("Field `PWM_0-10V_A` can't be converted to `f64`."))?,
        PWM_0-10V_B: fields
            .next()
            .ok_or_else(|| eyre!("Field `PWM_0-10V_B` not set."))?
            .raw_value_f64()
            .ok_or_else(|| eyre!("Field `PWM_0-10V_B` can't be converted to `f64`."))?,
    };
